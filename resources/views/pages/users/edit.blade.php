          ...
          <div class="card-body">
            ...
            <input type="hidden" class="form-control" id="id" name="id" value="{{ isset($user) ? $user->id : '' }}">
              ...
              <div class="form-group row">
                ...
                <div class="col-sm-9">
                  <select name="role" id="role" class="js-example-basic-single w-100 {{ $errors->has('role') ? ' is-invalid' : '' }}" required>
                    <option value="" 
                      @if (! isset($user)) 
                        selected 
                      @endif >Role</option>
                    <option value="1"
                      @if (isset($user)) 
                        ...
                        @elseif(old('old'))
                          @if(old('role') == 1) selected @endif 
                        @endif
                        >Admin</option>
                    <option value="0"
                      @if (isset($user)) 
                        @if($user->role_id == 0) selected @endif 
                      @elseif(old('old'))
                         @if(old('role') == 0) selected @endif
                      @endif 
                      >User</option>
                  </select>
                  @if($errors->has('role'))
